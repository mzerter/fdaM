...
if nargin < 5, 
    ...
    difeorder  = bwtdims(length(bwtdims));   
end
if nargin < 4, ufdcell = {};  end
if nargin < 3, awtcell = {};  end

difeorder = floor(difeorder);
if difeorder < 0, error('DIFEORDER is negative.');  end
...
if nvar == 1
    ...
    yarray = zeros([nfine,ncurve,difeordp1]);
    for j=1:difeordp1
    ...
    yprod = zeros([nfine,difeordp1,difeordp1]);
    for j1=1:difeordp1
        for j2=1:j1;
            if ncurve == 1
    ...
    if nforce > 0
        yuprod = zeros(nfine, nforce, difeordp1);
        for k=1:nforce
            for j1=1:difeordp1
                if ncurve == 1
    ...
    resmat  = eval_fd(tx, xfdcell{1}, difeorder);
    ...
    if difeorder > 1
        if any(size(bwtcell) ~= [nvar, nvar, difeorder])
            error('The dimensions of BWTCELL are incorrect.');
        end
    end
    ...
    yarray = zeros([nfine,ncurve,nvar,difeordp1]);
    for ivar1=1:nvar
        for j=1:difeordp1
    ...
    yprod = zeros([nfine,nvar,difeordp1,nvar,difeordp1]);
    for m1=1:nvar*difeordp1
        i1 = mmat(m1,1);
        j1 = mmat(m1,2);
        for m2=1:m1;
            i2 = mmat(m2,1);
    ...
    onesncurve = ones(1,ncurve);
    if nforce > 0
        yuprod = zeros(nfine, nvar, nforce, difeordp1);
        for k=1:nforce
            for i1=1:nvar
                for j1=1:difeordp1
                    if ncurve == 1
    ...
    mmat  = m2ij(nvar,difeorder);  %  order varies inside variables
    onesn = ones(nfine,1);
    ...
    if difeorder > 0
        barray = zeros(nfine,nvar,difeorder);  
    else
        barray = [];
    end
    ...
    for ivar1=1:nvar
        ...
        for k1 = 1:nforce
            ...
            if getestimate(afdPark1)
                ...
                indexi1 = mi11:mi12;
                %  DMAT entry for u-variable
                weightk1 = yuprod(:,ivar1,k1,difeordp1);
                dmat(indexi1) = trapzmat(abasismatk1, onesn, deltax, ...
                                         weightk1);
                %  add terms corresponding to x-derivative weights
                %  that are not estimated
                for m=1:nvar*difeorder;
                    i = mmat(m,1);
                    j = mmat(m,2);
                    if difeorder == 1
                        bfdParij   = bwtcell{ivar1,i};
        ...
        for m1=1:nvar*difeorder
            ...
            if getestimate(bfdParij1)
                ...
                indexij1 = mij11:mij12;
                %  DMAT entry 
                weightij1 = yprod(:,i1,j1,ivar1,difeordp1);
                dmat(indexij1) = ...
                    trapzmat(bbasismatij1,onesn,deltax,weightij1);
                %  add terms corresponding to forcing functions with
                %  unestimated coefficients
    ...
    for ivar1=1:nvar
        ...
        %  initialize with highest order derivative for this variable
        resmat  = eval_fd(tx, xfdcell{ivar1}, difeorder);
        %  add contributions from weighted u-functions
        for k=1:nforce
