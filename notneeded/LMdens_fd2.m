...
%  check zmat for containing the one vector, and, if so,
%  find a basis for the complement of sum(zmat)'

if ~isempty(zmat)
    %  check that the one vector is in the 
    %  column span of zmat
    onevec  = ones(nobs,1);
    onecoef = zmat\onevec;
    onefit  = norm(onevec-zmat*onecoef);
    if onefit/nobs > 1e-12
        error('One vector not in the column span of ZMAT.');
    end
    beta0 = zmat*
    %  compute a basis matrix for the complement of sum(zmat);
    ncov = size(zmat,2);
    Zsum = sum(zmat)';
    if ncov > 1
        [Q,R] = qr(Zsum);
        sumbas = Q(2:ncov);
        bcompvec0 = sumbas'*bvec0;
    else
        sumbas = [];
        bvec0  = sum(y)/Zsum;
    end
...
hmat = -EHessfnLM(basisobj, cvec0, U0, zmat, Cval, Ephi, sigma0);
...
for iter = 1:iterlim
    ...
    for stepiter = 1:STEPMAX
        ...
        if covwrd
            if ncov > 1
                betanew = beta + linemat(1,5).*sumbas*deltac(ind2);
            else
    ...
    hmat = EHessfnLM(basisobj, cvec, U, zmat, Cval, Ephi, sigma0);
    if lambda > 0
        hmat(ind1,ind1) = hmat(ind1,ind1) + 2.*Kmat;
    end
    if ninactive > 0 
...
if ~isempty(zmat)
    ...
    DWvec   = Dphimat*cvec;
    if ncov > 1
        Dlogl = [Dlogl; -(zmat*sumbas)'*DWvec./sigma];
    else
...
%  This code is for expected hessian
    ...
    EDcdlnL = -zmatbas'*onesn*EDphi'./sigma;
    ED2phi  = expect_phi(basisobj, cvec, Cval, 2);
    EDddlnL = zmatbas'*zmatbas.*(ED2phi'*cvec)./sigma^2;
