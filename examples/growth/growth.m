...
phdl = plot(agefine, hgtfmatfine, 'b-');
set(phdl, 'LineWidth', 2)
hold on
plot(age, hgtfmat(:,1:10), 'bo')
hold off
...
phdl = plot(agefine, velfmatfine, 'b-');
set(phdl, 'LineWidth', 2)
xlabel('\fontsize{16} Age')
...
subplot(1,1,1)
lhdl = plot(agefine, Dhgtfregmat(:,1:10), 'b-');
set(lhdl, 'LineWidth', 2)
lhdl = line(agefine, mean(Dhgtfregmat(:,1:10),2));
set(lhdl, 'LineWidth', 2, 'LineStyle', '--')
xlabel('Age (growth time)', 'FontSize', 16);
...
subplot(1,1,1)
lhdl = plot(agefine, warpmat(:,1:10), 'b-', [1,18], [1,18], 'k:');
set(lhdl, 'LineWidth', 2)
xlabel('\fontsize{16} Age (growth time)')
ylabel('\fontsize{16} Age (clock time)')
axis([1,18,1,18])
