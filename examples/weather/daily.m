...
betacell = cell(1,2);
lambda0   = 1e7;
betacell{1} = fdPar(smallbasis, harmaccelLfd, lambda0);

betabifd = bifd(zeros(65,65), smallbasis, smallbasis);
betacell{2} = bifdPar(betabifd, sLfd, tLfd, slambda, tlambda);

linmodstr = linmod(daytempfd, lnprecfd, betacell);

afd = linmodstr.alpha;   %  The intercept function
bfd = linmodstr.beta;     %  The bivariate regression function
